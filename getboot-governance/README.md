...
流量治理 starter，当前提供 Sentinel 接入抽象、统一配置前缀、Gateway 限流接入与 `@SentinelResource` 切面自动注册。

## 作用

- 统一治理能力入口，避免业务侧直接使用 Sentinel 原生前缀
- 自动补齐 `@SentinelResource` 注解切面
- 为 Spring Cloud Gateway 自动接入 Sentinel 过滤器与阻断处理器
- 为后续新增治理技术栈预留统一模块边界
...
- 需要继续扩展 Gateway、OpenFeign、RestTemplate、Actuator 等 Sentinel 桥接配置
...
- Gateway、OpenFeign、RestTemplate、Actuator 相关桥接只在对应组件存在时才有意义
...
- `getboot.governance.sentinel.gateway.*` 会映射为 `spring.cloud.sentinel.scg.*`
...
- 当前模块会在 Gateway 场景自动接入 `SentinelGatewayFilter` 与默认阻断处理器
