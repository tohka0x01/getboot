...
- [x] `getboot-mq`：当前 `9` 个测试；已补齐 RocketMQ/Kafka 配置桥接、Kafka 发送与监听 Trace 关键路径，并将 MQTT 发布链路切到 Spring Integration Paho 出站处理器验证，`2026-04-05` 已执行模块级 Maven `test`
...
- [x] `getboot-websocket`：当前 `4` 个测试；已补齐配置绑定、Jakarta WebSocket 自动配置与按会话/用户/广播推送主路径验证，`2026-04-05` 已执行模块级 Maven `test`
