...
- 模块自身已经依赖 `getboot-cache`、`getboot-coordination`、`getboot-limiter`，但默认处理器仍要求运行时存在 `StringRedisTemplate`、`RedissonClient` 与 `RateLimiter`
- 因此仍需要准备 Redis、Redisson 和限流对应配置；如果这些底层 Bean 不存在，默认 `WebhookRequestProcessor` 不会注册
