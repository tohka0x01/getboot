...
class PrometheusPropertyAliasEnvironmentPostProcessorTest {
    ...
    void shouldAliasPrometheusAndManagementProperties() {
        ...
        environment.getPropertySources().addFirst(new MapPropertySource("source", Map.of(
                "getboot.observability.prometheus.enabled", "true",
                "getboot.observability.management.endpoints.web.exposure.include", "health,info,prometheus"
        )));

        PrometheusPropertyAliasEnvironmentPostProcessor processor =
                new PrometheusPropertyAliasEnvironmentPostProcessor();
